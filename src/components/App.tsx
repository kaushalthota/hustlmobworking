...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleSetProfileTab = (event: any) => {
      if (event.detail && event.detail.tab === 'history') {
      ...
      const unsubscribe = notificationService.subscribeToUserNotifications(
        ...
        (notifications) => {
          ...
          const completionNotification = notifications.find(n => 
            !n.read && 
            n.type === 'info' && 
            n.content === 'Completed tasks are now in your Task History.'
          );
          
          if (completionNotification) {
            // Show task history notification if we're in the marketplace view
            if (currentView === 'marketplace') {
              // Mark as read
              notificationService.markAsRead(completionNotification.id);
            }
          }
        }
      );
              ...
              <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
                <div className="p-4 border-b flex justify-between items-center">
                  <h2 className="text-xl font-bold flex items-center">
                    <History className="w-6 h-6 text-[#0038FF] mr-2" />
                    Task History
                  </h2>
                  <button onClick={() => setShowTaskHistory(false)} className="text-gray-500 hover:text-gray-700">
                    <X className="w-6 h-6" />
                  </button>
                </div>
                <div className="p-6">
