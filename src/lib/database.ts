...
export const messageService = {
  ...
  async getUserChatThreads(userId: string): Promise<any[]> {
    try {
      ...
      if (querySnapshot.empty) {
        // Try the alternative query approach
        const altQ = query(collection(db, 'user_chats'));
        const allChats = await getDocs(altQ);
        
        // Filter manually for participants that include the userId
        const userChats = allChats.docs.filter(doc => {
          const data = doc.data();
        ...
        if (userChats.length === 0) {
          // Try one more approach - check if participants is stored as an object
          const objChats = allChats.docs.filter(doc => {
            const data = doc.data();
          ...
          if (objChats.length === 0) {
            return [];
          }
          
          // Map the chat threads to a more usable format
          const chatThreads = await Promise.all(objChats.map(async snapshotDoc => {
        ...
        // Map the chat threads to a more usable format
        const chatThreads = await Promise.all(userChats.map(async snapshotDoc => {
          const data = snapshotDoc.data();
